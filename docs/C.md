...
C언어 기반의 애플리케이션 대부분은 스택 영역과 힙 영역으로 이루어진 메모리를 갖는다. 

스택 영역 : 지역 변수가 선언되면 스택 영역에 순서대로 쌓이고 메모리에서 제거 될 때는 역순으로 제거된다. 스택 영역에 올라간 데이터는 호출이 종료되면 자동으로 해제된다. 그래서 스택 영역을 **자동 메모리**라고 부른다. 

힙 영역 : 힙은 **자유 저장소**라고 불린다. 자유 저장소는 프로그래머가 직접 메모리를 관리하는 메모리 영역이다. 예를 들어 ptr 이란 포인터 변수에 `malloc()`으로 메모리를 할당하는 경우 힙 영역에는 Pointer의 크기 만큼 공간이 할당된다. 스택 영역에는 ptr 변수가 생성되고 힙에 할당된 메모리를 가르키게 된다. 

ptr 변수는 함수 호출이 끝나면 자동으로 메모리에서 사라지지만 힙 영역에 할당된 메모리의 경우 프로그래머가 해제하지 않으면 사라지지 않고 남는다. 이런 식으로 사용되지 않는 메모리가 힙에 많아질 수록 사용할 수 있는 메모리는 줄어든다. 이것을 **메모리 누수**라고 한다. 
