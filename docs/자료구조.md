...
가장 마지막 원소인 `11`부터 확인한다. 리프 노드라면 힙 성질을 만족하는 것으로 간주해서 넘어가고 이전 노드를 확인한다. `12` 역시 리프 노드라 넘어간다. `3, 15, 10` 모두 건너뛰고 `1`이다. `1`은 자식 노드가 있기 때문에 자식 노드를 확인한다. `12`와 `11`이다. 자식 모두 `1` 보다 크다. 자식 중 더 큰 `12`와 `1`을 바꾼다. `A = [2, 8, 6, 12, 10, 15, 3, 1, 11]`이 된다. 자리를 바꾸고 `1`을 다시 확인한다. 리프 노드이기 때문에 넘어간다. 
...
새로 추가한 노드가 부모보다 작거나 루트 노드가 될 때까지 반복한다. 이렇게 하면 새로운 요소를 추가해도 힙 성질을 유지할 수 있다. 최악의 경우 새로 추가한 노드가 루트가 되는 경우기 때문에 `O(log n)`이다. 
...
리스트의 원소를 `Make Heap`을 사용해서 힙으로 만들고 가장 마지막 원소와 루트(가장 큰 원소)를 자리 바꿈한다. 리스트에서 가장 큰 값이 리스트의 맨 마지막 원소가 된다. 새로운 루트는 `heapify down`으로 제자리를 찾는다. 이런 식으로 리스트의 맨 마지막부터 맨 앞까지 순회하면서 정렬을 한다. `O(n log n)`이다.  
